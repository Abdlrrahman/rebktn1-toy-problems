...
function quickSort(arr, j, less, greater, pivot) {
  var less = less || [];
  var greater = greater || [];
  var i = i || 0;
  var j = j || 0;
  var pivot = pivot || arr[Math.floor(Math.random() * Math.floor(arr.length))];
      ...
      quickSort(arr, j + 1, less, greater, pivot)
  return arr;
}
...
//   var result = [];
//   var pivot = arr[Math.floor(Math.random() * Math.floor(arr.length))];console.log(pivot)
